...
class Mail_controller:
    ...
    def get_all_mail_sent_by_a_user(self, sender_id):
        """Function to retrieve all mail whose sender_id is the same as the logged in user_id"""
        sent = []
        # if len(sent)<1:
        #     return {"status":200, "message":"there are no sent mails yet by the current user"}
        for mail in mail_list:
            if mail['status']=="sent" and mail['sender_id']==sender_id:
                sent.append(mail)
        if sent:
            return {"status":200, "data":sent}
        return {"status":200, "message":"there are no sent mails yet by the current user"}
        if not mail_list:
            return {"status":200, "message":"There isn't any mail in the inbox"}
