...
class Database:
    ...
    def get_all_recieved_messages_of_a_user(self, reciever_id):
        ...
        """
        query = "SELECT * FROM messages WHERE status = read or status= sent and reciever_id ='{}'".format(reciever_id)
        self.cursor.execute(query)
        return self.cursor.fetchall()
    ...
    def modify_message_status(self, status, reciever_id, message_id):
        query = "UPDATE  inbox SET status = '{}' WHERE reciever_id = {} AND id = {}".format(status, reciever_id, message_id)
        self.cursor.execute(query)
        # return self.cursor.fetchone()
    def get_unread_mail_from_inbox(self, reciever_id):
        """get all unread from the inbox"""
        query = "SELECT * FROM inbox WHERE status = unread"
    ...
    def delete_mail(self, message_id, reciever_id):
        ...
        query = "DELETE FROM inbox WHERE id = {} AND reciever_id = {}".format(message_id, reciever_id)
        self.cursor.execute(query)
       
    def get_particular_message(self, message_id, reciever_id):
        query = "SELECT * FROM inbox WHERE message_id = {} AND reciever_id = {}".format(message_id, reciever_id)
        self.cursor.execute(query)
        return self.cursor.fetchall()
